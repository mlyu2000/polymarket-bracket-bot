...
# --- Supplement: best-ask (min) analysis. CLOB /book returns asks DESC-sorted,
# so asks[0] in the near-miss block above is the WORST ask and that block is dead.
# The detector itself correctly uses min(). ---
print()
print("=== SUPPLEMENT: best-ask (min) totals ===")
best = []
priced = 0
for m, (yes_book, no_book) in zip(result['mkts'], result['books']):
    if yes_book and no_book and yes_book.asks and no_book.asks:
        ...
        priced += 1
        if total <= 1.005:
            best.append((total, m.question[:60], yp, np_, m.liquidity))
best.sort()
print(f"priced markets (both books have asks): {priced}")
print(f"sum<1.00 on best asks: {len([b for b in best if b[0] < 1.00])}")
print(f"near-misses ≤1.005 on best asks: {len(best)}")
for total, q, yp, np_, liq in best[:5]:
    marker = "🟢" if total < 1.00 else "  "
    print(f"  {marker} {total:.4f} | Yes@{yp:.3f} No@{np_:.3f} | liq={liq} | {q}")
