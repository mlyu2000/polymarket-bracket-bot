    ...
    for m, (yes_book, no_book) in zip(markets, all_books):
        ...
        if opp:
            ...
            if yes_book and no_book and yes_book.asks and no_book.asks:
                ...
                # corrected: best (min) ask on each side; CLOB asks list is DESC-sorted
                ym = min(float(a['price']) for a in yes_book.asks)
                nm = min(float(a['price']) for a in no_book.asks)
                tm = ym + nm
                if floor_min is None or tm < floor_min:
                    floor_min = tm
                if tm <= 1.005:
                    near_misses_min.append((tm, m.question[:50], ym, nm, m.liquidity))
...
print(f"Near-misses ask[0] (<=1.005): {len(result['near_misses'])}")
for total, q, yp, np, liq in result['near_misses'][:5]:
    print(f"  {total:.3f} | Yes@{yp:.3f} No@{np:.3f} | {q}")
print(f"Near-misses min-ask (<=1.005): {len(result['near_misses_min'])} | priced={result['priced']} | floor={result['floor_min']}")
for total, q, yp, np, liq in result['near_misses_min'][:5]:
