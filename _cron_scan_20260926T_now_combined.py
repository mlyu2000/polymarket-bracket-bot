...
async def scan():
    ...
    opps = []
    near_misses = []          # provided method: asks[0] (worst ask, DESC-sorted)
    best_near = []            # supplement: true best ask = min(price)
    priced = 0
    for m, (yes_book, no_book) in zip(markets, all_books):
        ...
        if opp:
            ...
            if yes_book and no_book and yes_book.asks and no_book.asks:
                ...
                # Supplement: true best asks (CLOB asks are DESC-sorted, asks[0] = worst)
...
print(f"\n── Supplement: TRUE best-ask (min-of-asks) floor, {result['priced']} priced markets ──")
print(f"Best-ask floor sum: {result['best_near'][0][0]:.4f}" if result['best_near'] else "no priced books")
print(f"Near-misses ≤1.005 (true best ask): {len([x for x in result['best_near'] if x[0] <= 1.005])}")
for total, q, yp, np, liq in result['best_near'][:8]:
